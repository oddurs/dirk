...
fn generated_pages(
    ...
    if let Ok(roadmap) = std::fs::read_to_string(root.join("ROADMAP.md")) {
        // cairn writes an H1 and a "generated by" comment; the page has its own
        // title and does not need to be told twice.
        let body: String = roadmap
            .lines()
...
fn news_to_markdown(news: &str) -> String {
    let mut out = String::with_capacity(news.len());
